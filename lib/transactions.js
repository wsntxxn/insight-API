...
TxController.prototype.show = function(req, res) {
  var self = this;
  if (req.transaction) {
    if(req.transaction.blockheight <= self.LimitHeight)
      res.jsonp(req.transaction);
    else
      return self.common.handleErrors(new Error('Out of height limit!'), res);
  }
};
...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.bitcoind._height - transaction.height + 1;
  }
...
TxController.prototype.showRaw = function(req, res) {
  ...
  if (req.rawTransaction) {
    if (req.rawTransaction.blockheight <= self.LimitHeight)
      ...
      */
      ...
      async.mapSeries(txids, function (txid, next) {
        ...
        for(var i=0; i < transformed.length; ++i){
          if (transformed[i].blockheight <= self.LimitHeight) {
            showTxs.push(transformed[i]);
            showCount++;
          }
        }

        pagesTotal = Math.ceil(showCount / pageLength);
        res.jsonp({
          pagesTotal: pagesTotal,
          txs: showTxs
        });
