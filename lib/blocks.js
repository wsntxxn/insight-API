...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.bitcoind._height - blockCached.height + 1;
...
BlockController.prototype.show = function(req, res) {
  var self = this;
  if (req.block && req.block.height <= self.LimitHeight) {
    res.jsonp(req.block);
  }
  else return self.common.handleErrors(new Error('Out of limit!'),res);
};

BlockController.prototype.showRaw = function(req, res) {
  var self = this;
  if (req.rawBlock && req.block.height <= self.LimitHeight) {
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache && summaryCache.height <= self.LimitHeight) {
    finish(summaryCache);
  } else {
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      self.node.services.bitcoind.getBlockHeader(hash, function(err, blockHeader) {
        ...
        var confirmations = self.node.services.bitcoind._height - height + 1;
        if (confirmations >= self.blockCacheConfirmations) {
          self.blockSummaryCache.set(hash, summary);
        }

        if (summary.height <= self.LimitHeight) finish(summary);
        else finish(null);
      });
    });
    ...
    async.mapSeries(
      ...
      function(err, blocks) {
        ...
        for(var i = 0;i < blocks.length; ++i)
          if(blocks[i]!=null) ValidBlocks.push(blocks[i]);
        
        ValidBlocks.sort(function(a, b) {
          return b.height - a.height;
        });

        var data = {
          blocks: ValidBlocks,
          length: ValidBlocks.length,
          pagination: {
            next: next,
            prev: prev,
            currentTs: lte - 1,
